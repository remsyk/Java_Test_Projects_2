...
public class HeapAdaptablePriorityQueue<K,V> extends HeapPriorityQueue<K,V>
  ...
  public void decrementKey(K key, V value){
	  checkKey(key);                   // might throw an exception
	  Entry<K,V> temp = new AdaptablePQEntry<>(key, value, heap.size());
	  AdaptablePQEntry<K,V> locator = validate(temp);
	  int tempKey = (int) locator.getKey();
	  tempKey = tempKey - 1;
	  K tempkey2 = null;
	  HashMap<Integer, K> myMap = new HashMap<Integer, K>();
	  myMap.put(tempKey, tempkey2);
	  locator.setKey(tempkey2);             // method inherited from PQEntry
	  bubble(locator.getIndex());      // with new key, may need to move entry
	  System.out.println("Priority of process " + Integer.valueOf((String) temp.getValue()) + " decremented");	
  }
}
